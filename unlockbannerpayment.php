...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $dates = $row['dates'];
            $coinid = $row['securecoinid'];
            
        }
    if (!empty($coinid) && is_numeric($coinid)) {
        $query = "SELECT name FROM coins WHERE coin_id = $coinid";
        $result = $conn->query($query);
        $coinname = $result->fetch_assoc()["name"];
    }else{
        $coinname = '';
    }
    $conn->close();
            ...
            <div class="card p-2">
                <div class="card-body">
                    <table class="table " style="border-radius: 2px !important;">
                        ...
                        <tr>
                            <td>Coin:</td><td><?php echo $coinname ?></td>
                        </tr>
                    </table>
                    <table class="table">
                    ...
                    <div class="row">
                        ...
                        <div class="col ">
                            <form action="https://www.coinpayments.net/index.php" method="post" target="_top">
                                ...
                                <input type="hidden" name="item_name" value="Custom Profile Banner">
